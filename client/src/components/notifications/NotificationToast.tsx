...
const NotificationToast: React.FC<NotificationToastProps> = ({ notification, onDismiss }) => {
    ...
    useEffect(() => {
        // Stagger the appearance for a nice effect when multiple notifications appear
    ...
    const getNotificationStyles = () => {
        const baseStyles = "fixed transform transition-all duration-300 shadow-lg rounded-lg p-4 flex items-start max-w-sm";
        const visibilityStyles = isVisible
            ? "translate-y-0 opacity-100"
            : "translate-y-2 opacity-0";

        let typeStyles = "bg-blue-50 text-blue-800 border-l-4 border-blue-500"; // info default
